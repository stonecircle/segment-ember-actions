...
module.exports = function(context) {
    return {
        "FunctionExpression": function(node) {
            ...
            var analyticsTrackFound;
            node.body.body.forEach(function(expression) {
                if (expression.expression.callee.object.name === "analytics" && expression.expression.callee.property.name === "track") {
                    analyticsTrackFound = true;
                }
            });

            if (!analyticsTrackFound) {
                context.report(node, "no analytics.track() found");
            }
        }
    };
